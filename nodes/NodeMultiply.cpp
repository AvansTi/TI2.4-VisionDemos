...
void NodeMultiply::render()
{
    renderBegin("Multiply");
...
void NodeMultiply::compute(const NodeList& nodes)
{
    ...
    if (t1 == PinType::Image3)
    {
        ...
        if (t2 == PinType::Image3)
        {
            ...
            cv::multiply(img1, img2, image.mat);
        ...
        else if (t2 == PinType::Image1)
        {
            ...
            cv::Mat combined;
            cv::cvtColor(img2, combined, cv::COLOR_GRAY2BGR);
            cv::multiply(img1, combined, image.mat);
            outputPins[0].type = PinType::Image3;
        }
        else if (t2 == PinType::Int)
        {
            int v = (n2 ? n2->getPinInt(inputPins[1].connection) : value2);
            image.mat = img1 * (v/255.0f);
            outputPins[0].type = PinType::Image3;
        }
        else
    ...
    else if (t1 == PinType::Image1)
    {
        ...
        if (t2 == PinType::Image3)
        {
            ...
            cv::multiply(img1_3component, img2, image.mat);
            outputPins[0].type = PinType::Image3;
        }
        else if (t2 == PinType::Image1)
        {
...
cv::Mat NodeMultiply::getPinImage1(int pinId)
{
